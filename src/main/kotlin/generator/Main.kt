...
fun buildUpdate(state: State) {
    ...
    val entities = arrayOf(Klass::class, Grade::class, Student::class, Exam::class, SubjectKlassRel::class)
...
fun updateSlowyChangedVariable(state: State, updateCount: Int = ThreadLocalRandom.current().nextInt(1, 30)) {
    ...
    FileWriter("teacherUpdates${state.year}.sql").use { writer ->
        state.generateTeacherUpdateExpirience().forEachIndexed { index, teacher ->
            writer.appendln("UPDATE dbo.Teacher SET ExperienceYears = '${teacher.experienceYears}' WHERE ${Teacher.primaryKey} = '${teacher.pesel}'")
        }

        updatedTeachers.forEachIndexed { index, teacher ->
            writer.append("UPDATE dbo.Teacher SET Title = '${teacher.title}' WHERE ${Teacher.primaryKey} = '${teacher.pesel}'")
    ...
    FileWriter("studentsUpdates${state.year}.sql").use { writer ->
        updatedStudents.forEachIndexed { index, student ->
            writer.append("UPDATE dbo.Student SET Surname = '${student.surname}' WHERE ${Student.primaryKey} = '${student.pesel}'")
